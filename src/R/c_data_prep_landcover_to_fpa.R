...
if(!file.exists(file.path(extraction_anthro, 'fpa_landcover.rds'))) {
  if(!file.exists(file.path(extraction_anthro, 'fpa_w_all_landcover.gpkg'))) {
    ...
    system(paste("aws s3 sync", lf_s3, lf_path))
    
    df <- ext_landcover(rst_ = file.path(lf_path, lf_file), pts = fpa, colname = "lf_esp")
    
    st_write(df, file.path(proc_landcover_dir, lf_result_file))
    system(paste("aws s3 cp", file.path(proc_landcover_dir, lf_file), s3_proc_extractions))
